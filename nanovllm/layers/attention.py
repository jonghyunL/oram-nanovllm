...
# Print debug info only once to avoid log spam
_KVCACHE_DEBUG_PRINTED = True
...
def store_kvcache(key: torch.Tensor, value: torch.Tensor, k_cache: torch.Tensor, v_cache: torch.Tensor, slot_mapping: torch.Tensor):
    ...
    # Triton requires tl.arange ranges to be power-of-two. Use next pow2 block with masking.
    BLOCK = 1 << (D - 1).bit_length()
    global _KVCACHE_DEBUG_PRINTED
    if not _KVCACHE_DEBUG_PRINTED:
        print(f"[nanovllm][kv-cache] N={N} num_heads={num_heads} head_dim={head_dim} D={D} BLOCK={BLOCK}")
        _KVCACHE_DEBUG_PRINTED = True
