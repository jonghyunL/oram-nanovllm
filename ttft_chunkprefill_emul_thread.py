...
def main(model_type: str, input_len):
    ...
    def gpu_prefill_step(scheduled_reqs, has_prefill):
        # Uses llm_engine from outer scope
    ...
    def tokenizer_step(i, chunk_iter, prompt, sampling_params):
        # Also uses llm_engine and tot_scheduled_reqs from outer scope
        t0 = time.perf_counter()
        ...
        try:
            ...
            with ThreadPoolExecutor(max_workers=2) as pool:
                for i in range(1, chunk_iter+1):
                    ...
                    fut_gpu = pool.submit(gpu_prefill_step, scheduled_reqs, has_prefill)
                    fut_tok = pool.submit(tokenizer_step, i, chunk_iter, prompt, sampling_params)


                    prf_time = fut_gpu.result()
                    tok_time, next_scheduled_reqs, has_prefill = fut_tok.result()
                    torch.cuda.synchronize()
